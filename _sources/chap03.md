...
```{margin} A World-wide Community
A community of volunteer enthusiasts helped make the Python programming language itself a world-wide phenomenon. Watch this [short video](https://www.youtube.com/watch?v=WGCaK-N2dsA) to learn more.
```

Modern machines are connected through networks to an interesting and diverse world, if we learn how to access it, and our problems will be much easier to solve if we learn to leverage the libraries of code shared by the world-wide community of programmers and data scientists. This is where the real fun happens.
...
*   Understand the importance of the internationalization of software and what it means to localize it [design];
...
lines_in_Spanish = '''¡El gato en el sombrero!
Y él nos dijo:
...
**Decomposition to reduce the problem's complexity.** Before we try to solve this entire challenge in one go, let's ask if we can break it into simpler pieces. What do you think?

```{margin} Break It Down
...
**Strings as immutable sequences.** We'll begin by learning how to replace one word with another. We could code this functionality ourselves, but we have already seen that Python provides a number of useful methods that operate on sequences and Python strings are sequences. Before reading on, skim at the [Python documentation](https://docs.python.org/3/library/stdtypes.html#sequence-types-list-tuple-range) that lists the pre-programmed operations for sequences.
...
If we think back to the bookshelf example from the last chapter, this is like replacing this course's textbook with a different one (e.g., a new version of it). I haven't made a new copy of my bookshelf; I simply replaced one item in it with another. In code as in life, the old version of my bookshelf no longer exists.
...
Oops, this produces an error. The interpreter reports that a `str` object doesn't support item assignment. What it is trying to tell us is that, while a `str` is a sequence, it is not a mutable sequence. Python strings are *immutable* sequences, and thus we cannot use any of the methods for mutable sequence types. Once you create a string object, you cannot change the contents of that string. However, string processing is a common operation in programming and Python does provide [a whole set of methods we can apply to strings](https://docs.python.org/3/library/stdtypes.html#string-methods). And in this list we find [`replace`](https://docs.python.org/3/library/stdtypes.html#str.replace)!

Because Python strings are immutable, the `replace` method states that it creates and returns a new string object with the old subsequences replaced with the new subsequence. Make sure that you understand that `replace` left the original string object untouched when it created a new string object of the form we desired. We'll talk in a later chapter about why Python chose to make strings immutable, but for now we know enough to solve the first step in our challenge.
...
```{margin} Garbage Collection
You might wonder what happens to this now unnamed, original bookshelf sequence object. Great question! Briefly, there's a process that periodically finds all unnamed objects and recycles their storage. This is appropriately called _garbage collection_ in the computer science literature.
```

Line 10 of the script above applies the `replace` method to the object named `the_line`, and we take the object `replace` returns and update the name `the_line` to refer to it. Notice that we no longer have a way to access the string object that we read from `my_open_book`. That's ok, since we don't need that object any longer. 
...
```{admonition} You Try It
Try other substitutions starting with this code block. What else did you learn about how `replace` works from this example?
```
...
**Encode an emoji in Unicode.** Onto the second step, which has us substitute the string `'Gato'` with the Unicode code point for the cat-with-wry-smile emoji. You've already done something like this when we talked about how to express a carriage return. We used the special character sequence `'\n'` where the `'\'` told the Python interpreter that the next character wasn't a simple `'n'` but an encoding of something we couldn't express in a single printable character. We will do the same thing here.
...
Most of us don't find it easy to remember hexadecimal digits, and so there's one other escape sequence you might find useful.
...
**Functions.** Let's pretend that there is no string-replace method built into Python. How would we write a script that does what `replace4.py` does? Without access to Python's `replace` method, we would probably write some pseudocode that looks as follows. 
...
Probably not. Think about what happens if you find a mistake. You'd have to fix that mistake everywhere you copied the code. That is a bookkeeping nightmare. Furthermore, as the form of the pseudocode comments above illustrate, there is probably a lot of similarity in the two pieces of code. Both replace a specified substring in a string with a second substring. The only difference between the two pieces of code is the existing and new substrings specified. Wouldn't it be nice to write the code for our `replace` operation once and invoke that code whenever we needed it?
...
**Function definitions.** We are ready to expand our pseudocode to include the concept of a function definition. When you define a function, you should think about creating two important pieces: a *public interface* and a *hidden implementation*. 
...
```{margin} An Optional Count
The actual string `replace` method takes an optional `count` parameter. I leave it out of our discussion since we haven't used this optional functionality in our examples nor does it affect the point about interfaces abstracting away the details of the implementation.
```
...
**The actual function definition and its invocations.** Let's now fill in our pseudocode with actual Python code.
...
What is this name exactly? We previously talked about variable names, which permit us to give names to the values our script computes and manipulates. Function names serve a similar purpose, except that the value they name is a list of code statements that we need executed at one or more points in our script. For our problem, this need occurs on lines 26 and 27 of the previous code block.
...
A function's interface tells the Python interpreter, as well as humans reading the definition, how to invoke the function. To *invoke* (or *call*) a function, we use its name and specify what input values will be mapped to each of the function's formal parameters. These input values are called the *actual parameters*. Lines 26 and 27 illustrate two such calls with their parenthesized lists of actual parameters.

```{margin} Other Binding Mechanisms
Python supports other binding mechanisms, but we ignore these alternatives for now. If you really want to know the details at this point, I suggest you read [this section in the Python tutorial](https://docs.python.org/3/tutorial/controlflow.html#more-on-defining-functions).
```

When a function call is executed by the Python interpreter, each actual parameter is bound to its corresponding formal parameter. In the simplest case, this binding is done, as it is done here, in list order: the first actual parameter is bound to the first formal parameter; the second to the second; and so forth.
...
**Function execution.** It's important to realize that we have switched from talking about writing a function to a discussion of what happens when it executes. It might be helpful to state all together what happens when we tell the Python interpreter to execute `replace7.py`, which includes a function call. 
...
We won't step through every statement in the body of `my_replace`, but we should note that the `while` loop in this body eventually terminates because `s` (i.e., the first line read from the input file) has a finite length and this `while` loop increments `i` on every execution of its body (i.e., technically on every possible execution path through the loop's body). This means that the Python interpreter eventually reaches line 15, which is a new type of statement for us.
...
**Abstraction, decomposition, and algorithms.** Let's pause for a moment and rise above the details of building a function and learning how it executes. Our goal was to print a story from a file to our terminal screen while replacing occurrences of some words with some funny emoji. This short description pretty well sums up what our script does. It describes *the script's purpose*. 
...
```{tip}
Computer programs are not mystery novels. Computers do not like ambiguity and do not understand suspense. Make sure you give definitions to the names you use before you use them.
```

**Python's special variables.** A design pattern that we will frequently use to avoid this sort of problem is illustrated in `replace32.py` below. Pay particular attention to the two statements I appended to the very end of the script, and what I did to the statements that described the main action of the script. 
...
At the end of the script, I used one of those special names we saw in Chapter 1, when we looked at  Python's built-in `dir` command and discussed namespaces. The if-statement asks whether the special variable `__name__` is equal to the string `'__main__'`.  The Python interpreter sets `__name__` to this string if it is reading a script on which it was invoked. In other words, is this script the main thing it is being asked to do? Using our theater imagery, is it the start of the show? If it is, `__name__` has the value `'__main__'`. We'll see in a moment why the interpreter might give this special variable another value, but for now, this if-statement protects the invocation of a function called `main`.

In addition, I wrapped the "main" part of our script (formally lines 18-34 of `replace7.py`) in a function definition called `main`. So, instead of the Python interpreter starting execution with the input-statement, it simply remembers that statement as part of the function definition of `main`. By putting lines 39-40 as the last statements in our script, we can list `main` before `my_replace` or `my_replace` before `main`. Either way, both definitions will exist in the namespace before the interpreter gets to the computational work in our script.

```{tip}
Use this design pattern of wrapping your script's "main" work in a function called `main`, and protect its invocation with: `if __name__ == '__main__'`. You can then place `main` anywhere in your script. Some people like to start their Python scripts with the `main` routine, as everything after it are just details!
```

**Docstrings.** We added one other new line to `replace32.py`, and that is the funny-looking string on line 4. This is called a *docstring*, and it is simply a string literal that, as [PEP 257](https://www.python.org/dev/peps/pep-0257/) states, "occurs as the first statement in a module, function, class, or method definition." Its purpose is to explain what the function's interface leaves unsaid that is important in order for you or someone else to use this function in their code.
...
More generally, a docstring does not have to be a single line in length. You should think of the docstring as its formal specification. It outlines the contract between the person who wrote the function and the many people who will use it. John Guttag in his book on Python helpfully describes a function specification as having two primary pieces: (1) a description of the *assumptions* that the function makes about, for instance, its input parameters; and (2) the *guarantees* that the function provides if the assumptions are met.

**Getting a feel for abstraction.** Functions provide us with a particularly useful abstraction. With experience, you'll gain an intuitive sense for what makes a good procedure, i.e., a useful procedural abstraction. Let's start you on this experiential journey.
...
```{margin} Python's Power Operator
Put aside for a moment the fact that both these operations are concisely expressed in Python using its power operator **, which by the way illustrates that the designers of Python thought about this exact issue when deciding what operators to define!
```

Consider the following: If the problem we were trying to solve required us to compute both the square and cube of a number, should we create two separate functions, one to compute a number's square and another to compute its cube? Or instead, should we create a single function called power that can compute both answers? While our hypothetical square and cube functions would take only a single input parameter, our power function would take two parameters: a base and an exponent.
...
**Abstraction barriers.** We'll expand our discussion of abstraction, its beauty, and its power in Act II, but before leaving this topic, I want to put in a word of warning about building abstractions. Of all the statements in `my_replace`, the one that's easiest to get "wrong" is: `new_s = s[0:0]`.

```{margin} Exposing What's Behind the Curtain
See [https://www.youtube.com/watch?v=YWyCCJ6B2WE](https://www.youtube.com/watch?v=YWyCCJ6B2WE).
```

What do I mean by wrong? I mean we write something that looks like it should be correct, but it breaks what's called the *abstraction barrier*. The interesting thing about abstraction barriers is that there is no real barrier. Nothing stops us, when we write statements inside a function like `my_replace`, from reaching behind the abstraction we're trying to create and using operations that do not adhere to the abstraction. It's what Toto did in *The Wizard of Oz* when he pulled back the green curtain and exposed the man behind the Great and Powerful Oz. Let's think about what we don't want the Toto in each of us to expose in this statement. 
...
**Methods.** You may have wondered as we went through this exercise why the statement that used the built-in string-replace method looked like this:
...
Isn't that easy? The from-statement names a module, which is just a filename with the `.py` extension stripped off. It then lists what definitions we want to import from that module. This particular form of Python's import-statement places the imported definition names directly into the importing script's global symbol table, and that is why we did not have to make any changes to the function invocations later in the script.

This form works well if we want one or just a few definitions from a module. You will also use a form of the import-statement that imports the module name itself into a script's global symbol table. With this form, you can access any definition in that module by specifying its namespace. Here's what that looks like using our bookshelf example:
