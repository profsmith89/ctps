...
According to Brian Kernighan and P.J. Plauger, *"Everyone knows that debugging is twice as hard as writing a program in the first place."*[^fn1] Does that quote resonate with you?

Based on your experience, perhaps you think it underestimates the difficulty of debugging. It does when I consider how I typically spend my time. Nonetheless, Kernighan and Plauger follow up this general truth with a biting insight: *"So if you're as clever as you can be when you write it, how will you ever debug it?"*
...
Debuggers are programs that do exactly this.[^fn2] By learning how to use a debugger, you can do the following:

1. Start, stop, continue, and restart the execution of your script at any point in its execution.
...
You might even try commenting out lines 25−29 in `pdb2.py`, which calculate the correct amount of whitespace needed at the start of the breakpoint statement. Then rebuild `guess32-db.py` using this new version of `pdb2.py`. Next, run the `guess32-db.py` it produces to see how the error message changes.
...
```{admonition} You Try It
Look at the first code block above and do the following:
...
```{admonition} You Try It
Run `pdb3.py` on `guess32.py` adding a breakpoint at line 24. After you input your guess at the prompt, the execution of `guess32.py` will hit our inserted breakpoint, which gives execution control back to our debugging script. That's how `pdb3.py` can print the message that a breakpoint occurred at line 24. 
```
...
Since we don't want to reimplement what we already have in the Python debugger `pdb`, let's instead get a feel for accessing a script's runtime state by modifying `pdb3.py` so that it *instruments* a script. What does this mean? Basically, we'll build a script that allows us to specify: (1) where to put print-statements in a script; and (2) what we want those print-statements to say.

We've been doing this by hand and messing up our scripts along the way. An instrumentation tool could be a much cleaner approach. And it is trivial to do now that we have `pdb3.py`. We simply replace our breakpoint statement with a print-statement and have the user specify what they'd like printed.
...
The function `my_pin` is nearly an exact duplicate of `my_pdb`; the only two differences between them are the extra parameter `insert_code` and the lines 31−35 in `pdb3.py` that became a call to `insert_code` on line 44 in `pin32.py`. The point of these two changes is to remove what we want inserted from the work to set up the insertion. The function `my_pin` does the set-up work. It reads the script to be instrumented, locates where the new statements will be inserted, and computes the necessary leading whitespace. The passed-in function `insert_code` inserts the new statements at the located insertion point with the computed indentation.
...
To be even more efficient in my cheating, I could have stuck the print-statement after `guess32.py` generated the secret and before it asked for my guess (i.e., we would use `pin32.py` to insert a cheating print before line 18 in `guess32.py`). Cheating isn't our goal, but I hope you see that we could insert a debugging print using `pin32.py` anywhere in any script. No longer do we have to edit our scripts by hand to insert debugging prints!
...
```{admonition} You Try It
If you're so inspired, you might extend the functionality of `pin32.py`. Try, for example, wrapping `main`'s call to `my_pin` in a loop that allows you to add multiple instrumentation points into a target script. Make sure you translate the user-inputted line numbers into the correct index in the `edited_script` list as you process instrumentation points beyond the first.
```
