...
This chapter introduces you to the digital world that underlies all modern computation. It peeks below the useful abstractions we have replied upon in the previous chapters and teaches us that computers encode everything as numbers. We'll cover quite a bit of terminology while investigating what it takes to store a digital image and to identify objects in that image. In particular, by the end of the chapter, you will be able to:
...
Well, what might you, as a human, guess about the file's contents when you look at the filename `cosmo.jpg`? If you've ever worked with image files, the *filename extension* (i.e., that sequence of letters and numbers after the final period in the filename) is a great hint about what's in the file. By convention, the extension `.jpg` means that the file contains an image encoded in the [JPEG standard](https://docs.fileformat.com/image/jpeg/). 
...
Look at the top row of the hexdump in {numref}`Figure %s<c06_fig2_ref>`. After "Offset:", the first pair (i.e., the label on the leftmost column of the shaded 2D array) is `00` and the rightmost one `0F`. Now count the number of columns. I'll wait.
